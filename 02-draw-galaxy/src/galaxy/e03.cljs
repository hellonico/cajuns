...
(defn create-cloud [w h]
 {:x       (rand-range 0 w)
  :y       (rand-range 0 h)
  :radius  (rand-range 120 300)
  :opacity (rand-range 0.1 0.25)
  :dx      (rand-range (- (:x (:cloud-speed @settings))) (:x (:cloud-speed @settings)))
  :dy      (rand-range (- (:y (:cloud-speed @settings))) (:y (:cloud-speed @settings)))})
...
(defn spawn-comet [w h]
 (swap! comets conj
        {:x     (rand-range 0 w)
         :y     (rand-range 0 h)
         :angle (rand-range 0 (* 2 js/Math.PI))
         :speed (:comet-speed @settings)
         :tail  []}))

(defn update-comet [c dt]
 (let [{:keys [x y angle speed tail]} c
       nx (+ x (* speed dt (js/Math.cos angle)))
       ny (+ y (* speed dt (js/Math.sin angle)))
       newtail (conj tail [nx ny])
       clipped (if (> (count newtail) (:trail-length @settings))
                (subvec newtail 1)
                newtail)]
  (assoc c :x nx :y ny :tail clipped)))

(defn draw-comet [ctx {:keys [x y tail]}]
 (set! (.-strokeStyle ctx) "#fff")
 (set! (.-lineWidth ctx) 2)
 (.beginPath ctx)
 (doseq [[tx ty] tail] (.lineTo ctx tx ty))
 (.lineTo ctx x y)
 (.stroke ctx))
...
(defn animate [ctx w h timestamp]
 (let [dt (if (zero? @last-time) 0 (- timestamp @last-time))]
  ...
  (swap! comets (fn [cs] (mapv #(update-comet % dt) cs)))
  ...
  (draw-flares ctx dt)
  (doseq [c @comets] (draw-comet ctx c))
